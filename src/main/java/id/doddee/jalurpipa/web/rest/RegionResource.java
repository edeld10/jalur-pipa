...
/**
 * REST controller for managing {@link id.doddee.jalurpipa.domain.Region}.
 */
@RestController
@RequestMapping("/api")
public class RegionResource {
     ...
     * regionDTO, or with status {@code 400 (Bad Request)} if the region has already an ID.
     ...
     * regionDTO, or with status {@code 400 (Bad Request)} if the regionDTO is not valid, or with
     * status {@code 500 (Internal Server Error)} if the regionDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/regions")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of regions in
     * body.
     */
    @GetMapping("/regions")
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the regionDTO, or
